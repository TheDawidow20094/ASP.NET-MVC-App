...
namespace CarWorkshop.MVC.Tests
{
    public class CarWorkshopControllerTests : IClassFixture<WebApplicationFactory<Program>>
    {
        ...
        public async Task Index_ReturnsViewWithExpectedData_ForExistingWorkshops()
        {
            ...
            mediatorMock.Setup(c => c.Send(It.IsAny<GetAllCarWorkshopsQuery>(), CancellationToken.None)).ReturnsAsync(carWorkshops);

            var client = _factory.WithWebHostBuilder(builder => builder.ConfigureTestServices(services => services.AddScoped(_ => mediatorMock.Object))).CreateClient();
        ...
        [Fact()]
        public async Task Index_ReturnsEmptyViews_WhenNoCarWorkshopsExist()
        {
            //arrange
            var carWorkshops = new List<CarWorkshopDto>();
            var mediatorMock = new Mock<IMediator>();
